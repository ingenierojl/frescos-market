...
const NATIVE_AUTH_REDIRECT = "com.frescosmarket.app://auth-callback";

function isNativeApp() {
  return Boolean(window.Capacitor && window.Capacitor.isNativePlatform && window.Capacitor.isNativePlatform());
}

/* Solo dentro de la app nativa: cuando Chrome Custom Tab regresa via el
   deep link, aca se completa el login. En la web normal esto nunca se activa
   (window.Capacitor no existe), asi que no cambia nada para los navegadores. */
if (isNativeApp() && window.Capacitor.Plugins.App) {
  window.Capacitor.Plugins.App.addListener("appUrlOpen", async ({ url }) => {
    ...
    try {
      ...
      console.error("[auth] no se pudo completar el login nativo:", e);
    }
...
function ensureLoginModal() {
  ...
  overlay.querySelector('[data-provider="google"]').addEventListener("click", async () => {
    ...
    if (isNativeApp()) {
      // dentro de la app: pedimos la URL de login sin que Supabase intente
      // redirigir sola (Google bloquea el login embebido en WebViews), y la
      // abrimos nosotros en una Custom Tab forzada a Chrome.
      const { data } = await supabaseClient.auth.signInWithOAuth({
        provider: "google",
        options: { redirectTo: NATIVE_AUTH_REDIRECT, skipBrowserRedirect: true },
      });
      if (data && data.url && window.Capacitor.Plugins.ChromeCustomTab) {
        window.Capacitor.Plugins.ChromeCustomTab.open({ url: data.url });
      }
      return;
    }
